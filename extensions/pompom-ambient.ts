...
const BUNDLED_AMBIENT_DIR = path.join(MODULE_DIR, "ambient-audio");
...
/** Copy bundled ambient audio files to the user cache so playback works without an API key. */
function seedBundledAmbient(): void {
	try {
		if (!fs.existsSync(BUNDLED_AMBIENT_DIR)) return;
		fs.mkdirSync(AMBIENT_DIR, { recursive: true });
		const weathers: Weather[] = ["clear", "cloudy", "rain", "snow", "storm"];
		for (const w of weathers) {
			const src = path.join(BUNDLED_AMBIENT_DIR, `${w}.mp3`);
			const dest = path.join(AMBIENT_DIR, `${w}.mp3`);
			if (!fs.existsSync(src)) continue;
			// Don't overwrite if user already has a generated or custom file
			if (fs.existsSync(dest) && fs.statSync(dest).size > 1000) continue;
			// Also skip if a wav variant exists (generated by WAV-only backends)
			const destWav = path.join(AMBIENT_DIR, `${w}.wav`);
			if (fs.existsSync(destWav) && fs.statSync(destWav).size > 1000) continue;
			fs.copyFileSync(src, dest);
		}
	} catch { /* non-fatal — generation fallback still works */ }
}
